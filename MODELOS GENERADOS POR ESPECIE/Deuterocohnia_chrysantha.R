...
pres.comp<-read.table("~/especies/deuterocohnia_chrysantha/deuterocohnia_chrysantha.csv",header=T, sep=',', fill=TRUE, check.names=TRUE, stringsAsFactors=FALSE)
presencia<-pres.comp
...
write.csv(presencia, file = "~/especies/deuterocohnia_chrysantha/presencia&variables.csv")
...
write.table(presencia, file="~/especies/deuterocohnia_chrysantha/deuterocohnia_chrysantha_2.csv", sep=",", row.names=FALSE, quote=FALSE)
presencia
...
occs <- read.csv("~/especies/deuterocohnia_chrysantha/deuterocohnia_chrysantha_2.csv") #Datos de presencia
...
me <- maxent(oc.bg, y, args=c("addsamplestobackground=true"), 
             path="C:/Users/juand/Desktop/mxent/deuterocohnia_chrysantha/outputR2")

#contribucion por variable en el modelo Maxent 
var.importance(me)
me_df <- var.importance(me)
write.table(me_df, file="~/GEO/PUCV y we· 2020/plots finales/contribuciÛn variables/deuterocohnia_chrysantha.csv", sep=",", row.names=FALSE, quote=FALSE)
...
writeRaster(map,"C:/Users/juand/Desktop/mxent/deuterocohnia_chrysantha/deuterocohnia_chrysantha.tif",overwrite=T)
save(me,file="C:/Users/juand/Desktop/mxent/deuterocohnia_chrysantha/outputR2/mx_obj.RData")
...
load("C:/Users/juand/Desktop/mxent/deuterocohnia_chrysantha/outputR2/mx_obj.RData")
...
fold <- kfold(oc, k=4) #Genera un indice aleatorio de los folds
...
me_env <- maxent(env.values, y_env, args=c("addsamplestobackground=true"), path="C:/Users/juand/Desktop/mxent/deuterocohnia_chrysantha/outputR3")
...
#Now, for all folds
auc<-rep(NA,4) #crea vector vacÌas para guardar valores de AUC y umbrales
max.tss<-rep(NA,4)
for (i in 1:4){ #cambiar el numero de ciclo de acuerdo a los n∞ de k
  occtest <- oc[fold == i, ]
  occtrain <- oc[fold != i, ]
  env.values<-data.frame(rbind(occtrain, bg))
  y_env<-c(rep(1,nrow(occtrain)), rep(0,nrow(bg)))
  me_env2 <- maxent(env.values, y_env, args=c("addsamplestobackground=true"),
               path="C:/Users/juand/Desktop/mxent/deuterocohnia_chrysantha/outputR4")
...
plot(map, main = "Deuterocohnia chrysantha")
points(occs, pch='+', cex=0.5, col='black')
umbral <- (map >= umbral.tss)
class(umbral)
writeRaster(umbral,"C:/Users/juand/Desktop/mxent/deuterocohnia_chrysantha/deuterocohnia_chrysantha_umbral.tif",overwrite=T)
...
#correlaciones 
corr_var <- read.csv("~/especies/deuterocohnia_chrysantha/presencia&variables.csv")
corr_var
corr_var <- corr_var[,5:ncol(corr_var)]
