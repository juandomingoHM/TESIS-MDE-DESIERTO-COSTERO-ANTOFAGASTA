...
pres.comp<-read.table("~/especies/monttea_chilensis/monttea_chilensis.csv",header=T, sep=',', fill=TRUE, check.names=TRUE, stringsAsFactors=FALSE)
presencia<-pres.comp
...
write.csv(presencia, file = "~/especies/monttea_chilensis/presencia&variables.csv")
...
write.table(presencia, file="~/especies/monttea_chilensis/monttea_chilensis_2.csv", sep=",", row.names=FALSE, quote=FALSE)
presencia
...
occs <- read.csv("~/especies/monttea_chilensis/monttea_chilensis_2.csv") #Datos de presencia
...
me <- maxent(oc.bg, y, args=c("addsamplestobackground=true"), 
             path="C:/Users/juand/Desktop/mxent/monttea_chilensis/outputR2")

#contribucion por variable en el modelo Maxent 
var.importance(me)
me_df <- var.importance(me)
write.table(me_df, file="~/GEO/PUCV y we· 2020/plots finales/contribuciÛn variables/monttea_chilensis.csv", sep=",", row.names=FALSE, quote=FALSE)
...
ggplot(map_df, aes(x=layer)) +
  ...
  theme_classic() + scale_y_continuous(breaks=seq(0, 250000, 50000))

#Guardar los resultados
writeRaster(map,"C:/Users/juand/Desktop/mxent/monttea_chilensis/monttea_chilensis.tif",overwrite=T)
save(me,file="C:/Users/juand/Desktop/mxent/monttea_chilensis/outputR2/mx_obj.RData")
...
load("C:/Users/juand/Desktop/mxent/monttea_chilensis/outputR2/mx_obj.RData")
...
fold <- kfold(oc, k=3) #Genera un indice aleatorio de los folds
...
me_env <- maxent(env.values, y_env, args=c("addsamplestobackground=true"), path="C:/Users/juand/Desktop/mxent/monttea_chilensis/outputR3")
...
#Now, for all folds
auc<-rep(NA,3) #crea vector vacÌas para guardar valores de AUC y umbrales
max.tss<-rep(NA,3)
for (i in 1:3){ #cambiar el numero de ciclo de acuerdo a los n∞ de k
  occtest <- oc[fold == i, ]
  occtrain <- oc[fold != i, ]
  env.values<-data.frame(rbind(occtrain, bg))
  y_env<-c(rep(1,nrow(occtrain)), rep(0,nrow(bg)))
  me_env2 <- maxent(env.values, y_env, args=c("addsamplestobackground=true"),
               path="C:/Users/juand/Desktop/mxent/monttea_chilensis/outputR4")
...
plot(map, main = "Monttea chilensis")
points(occs, pch='+', cex=0.5, col='black')
umbral <- (map >= umbral.tss)
class(umbral)
writeRaster(umbral,"C:/Users/juand/Desktop/mxent/monttea_chilensis/monttea_chilensis_umbral.tif",overwrite=T)
...
#correlaciones 
corr_var <- read.csv("~/especies/monttea_chilensis/presencia&variables.csv")
corr_var
